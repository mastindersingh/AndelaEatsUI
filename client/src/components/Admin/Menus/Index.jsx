...
import { func, shape, arrayOf, bool, any } from 'prop-types';
...
import { fetchUpcomingEngagements } from '../../../actions/admin/engagementsAction';
import {
  formatDate,
...
export class Menus extends Component {
   ...
   */
  deleteMenu = menuId => {
    this.props.deleteMenuItem(menuId).then(() => this.closeModal());
  };
  ...
  handleSubmit = menu => {
    if (!this.state.editMenu) {
      this.props.createMenu(menu).then(() => {
        this.closeModal();
      });
    } else {
      const { id, mealPeriod } = this.state.menuDetails;
      this.props
        .editMenu({ ...menu, id, mealPeriod: mealPeriod.toLowerCase() })
        .then(() => {
          this.closeModal();
        });
    }
